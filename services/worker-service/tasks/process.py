...
from tasks.index   import index_chunks, update_document_status

from sqlalchemy import create_engine, text
import os

# Sync URL — strip asyncpg driver if present
...
def process_document(self, document_id: str):
    ...
    Chains all 4 steps: extract → chunk → embed → index
    """
    print(f"\n{'='*50}")
    ...
    try:
        ...
        chunks = chunk_pages(
            ...
            domain_id=domain_id,
            model=get_model(),      # model loaded once on worker startup
        )
        ...
        indexed_count = index_chunks(chunks_with_vectors)

        # ── Mark as done ──────────────────────────────────────────
        update_document_status(document_id, "done")

        print(f"\n✓ Document {document_id} processed successfully")
        print(f"  {indexed_count} chunks indexed into domain '{domain_id}'")
        print(f"{'='*50}\n")
