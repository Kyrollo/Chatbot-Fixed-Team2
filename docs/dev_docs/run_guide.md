...
> [!NOTE]
> `scripts/run_migration.py` uses smart schema parsing. It executes `migrations/setup_all.sql`. If Apache AGE is not detected (i.e., you haven't set up WSL2), it automatically skips graph ontology commands (`create_graph`, AGE-specific statements) and successfully initializes all relational schemas without failing.
...
DATABASE SETUP (PostgreSQL 17 on port 5434):
  ...
  python scripts/run_migration.py          # create tables + seed
